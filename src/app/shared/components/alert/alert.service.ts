...
export class AlertService {
    ...
    constructor( router : Router ) {
        router.events.subscribe( event => {
                                         if( event instanceof NavigationStart ) // se event é uma instancia de NavigationStart, estou gerando uma nova navegação
                                                                                {
    ...
    private alert( alertType: AlertType, message: string, keepAfterRouteChange: boolean ) {
        ...
        this.alertSubject.next( new Alert( alertType, message ) );  // aqui eu estou emitindo um novo objeto, novo alerta
    }
